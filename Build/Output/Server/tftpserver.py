...
# TFTP Performance Tuning:
# vmbr2 bridge has MTU 9000 (jumbo frames) — use large block size
# Block size 8192: fits in jumbo frame (9000 MTU - 20 IP - 8 UDP - 4 TFTP = 8968 max)
# Window size 64: send 64 blocks per ACK = 512KB per window burst
# Theoretical: ~50+ MB/s on internal Proxmox bridge
BLOCK_SIZE = 1468
...
class TransferSession:
    ...
    def __init__(self, client_addr: Tuple[str, int], filepath: str, filesize: int, block_size: int = 1456, window_size: int = 1):
...
class TFTPServer:
    ...
    def _create_socket(self, port: int) -> Optional[socket.socket]:
        ...
        try:
            ...
            # 8MB send/receive buffers for burst window transfers
    ...
    def _handle_read_request(self, data: bytes, addr: Tuple[str, int]):
        ...
        # Handle windowsize option (RFC 7440 - send multiple blocks per ACK)
        window_size = 1
        if "windowsize" in options:
            requested_ws = int(options["windowsize"])
            window_size = min(max(requested_ws, 1), MAX_WINDOW_SIZE)

        self.logger.info(f"[{addr[0]}] RRQ {filename} ({filesize} bytes, blksize={block_size}, window={window_size})")

        # Create session
        session = TransferSession(addr, filepath, filesize, block_size, window_size)
        session.open()
    ...
    def _send_oack(self, addr: Tuple[str, int], block_size: int, filesize: int, options: dict):
        ...
        if "windowsize" in options:
            ...
            ws = session.window_size if session else 1
            pkt += b"windowsize\x00" + str(ws).encode() + b"\x00"
    ...
    def _send_next_block(self, addr: Tuple[str, int]):
        """Read and send the next window of data blocks."""
        with self._lock:
            session = self.sessions.get(addr)
        ...
        # Send up to window_size blocks before waiting for ACK
        for _ in range(session.window_size):
