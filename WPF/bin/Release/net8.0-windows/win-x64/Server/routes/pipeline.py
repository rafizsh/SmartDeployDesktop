...
async def register_client_event(evt: PxeClientEvent):
    ...
    if evt.event == "pipeline_start":
        client["current_step"] = "Starting deployment"
        client["progress"] = 0
    elif evt.event == "pipeline_step":
        # Detail format: "N/T : step name" where N is current step, T is total.
        # Also accepts legacy "N: step name" format and falls back gracefully.
        try:
            prefix = evt.detail.split(":", 1)[0].strip() if ":" in evt.detail else evt.detail
            if "/" in prefix:
                step_num, total = prefix.split("/", 1)
                step_num = int(step_num)
                total = max(int(total), 1)
                client["progress"] = round((step_num / total) * 100)
            else:
                # Legacy format - assume old 19-step pipeline
                step_num = int(prefix)
